...
typedef uint64_t Key; // Khóa Zobrist cho bảng băm

enum Color { WHITE, BLACK, COLOR_NB = 2 };
enum PieceType { NO_PIECE_TYPE, PAWN, KNIGHT, BISHOP, ROOK, QUEEN, KING, PIECE_TYPE_NB = 8 };
enum Piece { NO_PIECE, W_PAWN, W_KNIGHT, W_BISHOP, W_ROOK, W_QUEEN, W_KING,
             B_PAWN, B_KNIGHT, B_BISHOP, B_ROOK, B_QUEEN, B_KING, PIECE_NB = 14 };
enum Square {
...
enum Direction { NORTH = 8, SOUTH = -8, EAST = 1, WEST = -1,
                NORTH_EAST = 9, NORTH_WEST = 7, SOUTH_EAST = -7, SOUTH_WEST = -9 };

// Giá trị cho bảng băm
enum Bound { BOUND_NONE, BOUND_UPPER, BOUND_LOWER, BOUND_EXACT };

// Hàm kiểm tra Square hợp lệ
inline bool is_ok(Square s) { return s >= SQ_A1 && s <= SQ_H8; }

// Hàm lấy file và rank từ Square
inline File file_of(Square s) { return File(s % 8); }
inline Rank rank_of(Square s) { return Rank(s / 8); }

// Hàm tạo Square từ file và rank
