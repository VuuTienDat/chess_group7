    ...
    for (int depth = 1; depth < 1000; depth++) {
        ...
        for (; result_moves_gen.next();) {
            ...
            move_str =
                ...
                (move.prom != ' ' ? std::string(1, static_cast<char>(tolower(move.prom))) : "");

            printPvInfo(move, depth, score, start_time, flip_side);

            if (deltaMs(Clock::now(), start_time) > ms_time) {
                stop_search = true;
                break;
            }
        }
...
void Searcher::printPvInfo(Move                                  move,
    ...
    // Sửa: Thêm kiểm tra để tránh chia cho 0 khi tính NPS
    int nps = (time > 0) ? (this->nodes_searched * 1000) / time : 0;
